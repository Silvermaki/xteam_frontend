...
export class MyRequestsService {
  ...
  //get_user_requests function
    get_user_requests(payload: any):Observable<any>{
        let bodyString = JSON.stringify(payload);
        let headers = new Headers({ 'Content-Type': 'application/json' });
        let options = new RequestOptions({ headers: headers });
        return this.http.post(this.baseUrl+"/get_user_requests",bodyString, options).map(this.extractData).catch(this.handleError);
    }
