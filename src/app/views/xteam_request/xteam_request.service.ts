...
@Injectable()
export class XteamRequestService {
  ...
  //newRequest function calls new_request endpoint
  newRequest(payload: any):Observable<any>{
    let bodyString = JSON.stringify(payload);
    let headers = new Headers({ 'Content-Type': 'application/json' });
    let options = new RequestOptions({ headers: headers });
    return this.http.post(this.baseUrl+"/new_request",bodyString, options).map(this.extractData).catch(this.handleError);
  }
